...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (!MQTT_WS_URL || !MQTT_USER || !MQTT_PASS) {
      console.error("Environment variables for MQTT are missing.");
      return;
    }
  ...
  const handleOpenControlModal = (id: string) => {
    ...
    if (registeredDevices[id]) {
      setModalNameInput(registeredDevices[id].nome);
    }
  };
  ...
  const handleTogglePump = (id: string, currentState: boolean) => {
  ...
  const sortedDevices = Object.values(registeredDevices).sort((a, b) => {
    if (a.status === b.status) return a.nome.localeCompare(b.nome);
    return a.status === "Online" ? -1 : 1;
  });

  const sortedModalIds = Object.keys(discoveredDevices).sort((a, b) => {
    const aRegistered = !!registeredDevices[a];
    const bRegistered = !!registeredDevices[b];
    ...
    >
        ...
        {sortedDevices.length === 0 ? (
          ...
          >
            ...
            <p className="text-sm text-slate-500 mt-1">
              Utilize o botão "Adicionar Dispositivo" para iniciar o
              monitoramento dos aparelhos disponíveis.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {sortedDevices.map((disp) => (
              <div
                ...
                className={`group border rounded-2xl p-5 select-none transition-all duration-300 transform shadow-sm cursor-pointer hover:-translate-y-1 ${
                  isDarkMode
                    ? disp.status === "Online"
                      ? "bg-slate-900/60 border-slate-800 hover:border-slate-700 hover:bg-slate-900"
                      : "border-slate-900 bg-slate-950/40 opacity-60 hover:border-slate-800"
                    : disp.status === "Online"
                      ? "bg-white border-slate-200 hover:shadow-md hover:border-slate-300"
                      : "border-slate-200 bg-slate-100/60 opacity-60 hover:shadow-sm"
                } ${selectedId === disp.id ? "border-emerald-500 ring-2 ring-emerald-500/10" : ""}`}
              >
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h3
                      className={`font-bold transition-colors truncate max-w-[180px] text-base group-hover:text-emerald-500 ${
                  ...
                  >
                    <span
                      className={`w-1.5 h-1.5 rounded-full ${disp.status === "Online" ? "bg-emerald-500 animate-pulse" : "bg-slate-400"}`}
                    />
                    {disp.status}
                  </span>
                </div>
                ...
                >
                  ...
                  <div>
                    ...
                    <strong
                      className={`text-sm ${disp.status === "Online" && disp.bomba ? "text-emerald-600" : "text-slate-400 dark:text-slate-600"}`}
                    >
                      {disp.status === "Online"
                        ? disp.bomba
                          ? "Ativo"
      ...
      {selectedId && activeDevice && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm transition-opacity duration-300">
          <div
            className={`w-full max-w-md rounded-2xl p-6 shadow-2xl border transition-all duration-300 animate-in fade-in zoom-in-95 ${
              isDarkMode
          ...
          >
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className={`flex-1 hover:cursor-pointer py-2.5 rounded-xl font-bold text-xs transition-all text-white ${
                      activeDevice.bomba || activeDevice.status === "Offline"
                        ? "bg-slate-200 dark:bg-slate-800 text-slate-400 dark:text-slate-600 cursor-not-allowed opacity-40 shadow-none"
            ...
            <div
              className={`border-t pt-4 mt-6 flex flex-col gap-3 ${isDarkMode ? "border-slate-800" : "border-slate-100"}`}
            >
              <button
                onClick={() => setIdToRemove(activeDevice.id)}
                className="w-full hover:cursor-pointer py-2 bg-transparent hover:bg-rose-50 dark:hover:bg-rose-950/20 text-rose-600 text-xs rounded-xl transition-all font-bold border border-transparent   "
              >
      ...
      {isPairingOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm transition-opacity duration-300">
          <div
            className={`w-full max-w-md p-6 rounded-2xl shadow-2xl border text-slate-800 transition-all duration-300规范 animate-in fade-in zoom-in-95 ${
              isDarkMode
                ? "bg-slate-900 border-slate-800 text-slate-100"
                : "bg-white border-slate-200"
            }`}
          >
            ...
            {sortedModalIds.length === 0 ? (
              <div className="py-6 text-center text-slate-400 text-xs">
                Procurando por dispositivos conectado a rede...
              </div>
            ) : (
      ...
      {/* MODAL REMOVER APARELHO */}
      {idToRemove && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm transition-opacity duration-300">
          <div
            className={`w-full max-w-sm p-6 rounded-2xl shadow-2xl text-center border transition-all duration-300 animate-in fade-in slide-in-from-bottom-10 ${
              isDarkMode
                ? "bg-slate-900 border-slate-800 text-slate-100"
                : "bg-white border-slate-200 text-slate-800"
            }`}
